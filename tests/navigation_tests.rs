...
fn j_key_scrolls_down_left_panel_in_dashboard() {
    ...
    assert_eq!(state.scroll_offsets.task_list, 6);
}
...
fn j_key_scrolls_right_panel_when_focused_in_dashboard() {
    ...
    handle_key(&mut state, key(KeyCode::Char('j')));
    assert_eq!(state.scroll_offsets.event_stream, 11);
}
...
fn k_key_scrolls_right_panel_when_focused_in_dashboard() {
    ...
    state.scroll_offsets.event_stream = 10;
    handle_key(&mut state, key(KeyCode::Char('k')));
    assert_eq!(state.scroll_offsets.event_stream, 9);
}
...
fn j_key_scrolls_agent_events_in_right_panel() {
    ...
    handle_key(&mut state, key(KeyCode::Char('j')));
    assert_eq!(state.scroll_offsets.agent_events, 9);
}
...
fn filter_mode_char_appends_to_filter() {
    ...
    assert_eq!(state.filter.as_deref(), Some("tes"));
}
...
fn filter_mode_enter_keeps_filter() {
    ...
    state.filter = Some("test".to_string());
    handle_key(&mut state, key(KeyCode::Enter));
    assert_eq!(state.filter.as_deref(), Some("test"));
}
...
fn unknown_key_in_normal_mode_is_noop() {
    ...
    assert!(matches!(state.view, ViewState::Dashboard));
    assert!(!state.should_quit);
    assert_eq!(0, state.scroll_offsets.task_list);
}
...
fn multiple_waves_drill_down_correct_task() {
    ...
    state.agents.insert("a03".into(), Agent::new("a03".into(), now));
    state.recompute_sorted_keys();
    ...
    state.selected_task_index = Some(2); // Third task → agent a03
...
fn navigation_preserves_unrelated_state() {
    ...
    handle_key(&mut state, key(KeyCode::Char('3')));
    assert_eq!(state.scroll_offsets.sessions, 5);
...
fn filter_mode_prevents_navigation() {
    ...
    state.filter = Some("test".to_string());
    handle_key(&mut state, key(KeyCode::Char('3')));
    assert_eq!(state.filter.as_deref(), Some("test3"));
    assert!(matches!(state.view, ViewState::Dashboard));
}
