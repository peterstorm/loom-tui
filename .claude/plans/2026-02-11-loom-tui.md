...
**Decomposed into sub-states (addresses PR #3 review C3):**
...
**Wave 4 -- ✓ COMPLETED (PR #3)**

| Task | Description | Files | Status |
|------|-------------|-------|--------|
| P4-T1 | Dashboard view: header, footer, wave_river, task_list, event_stream. Layout with 35/65 split. Banner component. | `src/view/dashboard.rs`, `src/view/components/{header,footer,wave_river,task_list,event_stream,banner}.rs` | ✓ |
| P4-T2 | Agent detail view: **selectable agent_list + filtered event stream** (replaced original tool_call_list/reasoning_panel design). | `src/view/agent_detail.rs`, `src/view/components/{agent_list,event_stream}.rs` | ✓ |
| P4-T3 | Sessions browser + session detail: session list, session detail view with archived state, filter_bar, help_overlay. View dispatch in view/mod.rs. | `src/view/{sessions,session_detail}.rs`, `src/view/mod.rs`, `src/view/components/{filter_bar,help_overlay}.rs` | ✓ |

### Phase 5: Integration (depends on all previous phases)

**Wave 5 -- ✓ COMPLETED (PR #3)**
...
| P5-T2 | End-to-end testing: manual smoke tests, integration test coverage for all modules. | `tests/{main_loop,navigation,session,update,view}_tests.rs` | ✓ |

---

## Post-Implementation: Code Quality & Review Fixes

**PR #3 merged** (56 files, 15.9k additions, full TUI implementation)

**PR #5 -- Review Fixes (in progress):**

| Phase | Description | Status |
|-------|-------------|--------|
| Phase 1 | Clippy warnings + silent send failure handling | ✓ |
| Phase 2 | Replace stringly-typed errors with thiserror enums | ✓ |
| Phase 3 | Decompose AppState god object into UiState/DomainState/AppMeta | ✓ |
| Phase 4 (planned) | Make fields pub(crate), route mutations through methods | Pending |
| Phase 5 (planned) | Extract pure functions from large impl blocks | Pending |
